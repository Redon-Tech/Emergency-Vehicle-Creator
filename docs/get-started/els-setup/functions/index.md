...
## Patterns and activations

Each function contains patterns and activations. Patterns contain the actual flasher, fader, and rotator data. Activations decide how the function changes patterns.

The default vehicle configuration includes **Stages** and **Traffic Advisor**. Their default keybinds are J/DPadLeft and K/DPadRight.

A function also has a **weight**. A higher weight has priority when more than one function uses the same light.

See [Activations](activations.md) for the activation types and [Flashers](../../patterns/flashers.md) to start making a pattern.